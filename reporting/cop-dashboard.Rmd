...
### Workshop Overall Assessment Scores: Excellent + Very Good (Average All Workshops)
...
### Workshop Organization Assessment Scores: Excellent + Very Good (Average All Workshops)
...
metric_organization <- workshop_feedback %>% 
  mutate(metric = event_organization_excellent_percent + event_organization_very_good_percent) %>% 
  select(metric) %>% 
  summarise(percent = mean(metric, na.rm =TRUE)) %>% 
  mutate(percent = paste0(percent, "%")) %>% 
  pull()
...
### Workshop Overall Assessment Scores by Topic

```{r}
tidy_feedback <- renamed_feedback %>%
  pivot_longer(cols = starts_with("overall"), names_to = "category", values_to = "percent") %>%
  select(event_title, category, percent) %>% 
  mutate(cat_label = stringr::str_remove(category, "overall_assessment_"),
         ...
         cat_label = factor(cat_label, levels = c("excellent",
                                                  ...
                                                  "poor"))) 
#colour palette
colour_count <-  length(unique(tidy_feedback$category))
get_palette <-  colorRampPalette(brewer.pal(8, "YlGn"))
...
### Workshop Organization Assessment Scores by Topic
