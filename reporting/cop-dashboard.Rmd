...
### Average Excellent/Very Good Event Organization Rating

```{r}
metric_org <- workshop_feedback %>% 
...
gauge(metric_org, min = 0, max = 100, symbol = '%', gaugeSectors(
  success = c(80, 100), warning = c(40, 79), danger = c(0, 39)
))

# valueBox(value = metric_org, icon = "fa-commenting-o",
#          caption = "Excellent or Very Good Organization Rating (All Workshops)")
```


### Average Excellent/Very Good Overall Event Rating


```{r}
metric <- workshop_feedback %>% 
...
gauge(metric, min = 0, max = 100, symbol = '%', gaugeSectors(
  success = c(80, 100), warning = c(40, 79), danger = c(0, 39)
))

# valueBox(value = metric, icon = "fa-comments",
...
```{r}
workshop_attendance <- workshop_feedback %>% 
  mutate(event_title = recode(event_title,
                              "bcgov-user-day" = "data science demo day",
                              "bcgov-data-science-demo-day-workshop" = "data science demo day",
                              "data-vis-workshop" = "data visualisation",
                              "intro to ds using r" = "intro to data science programming",
                              "flnr-r-geospatial-workshop" = "geospatial data science "
                              )) %>% 
  group_by(event_title) %>% 
  summarise(total_trained = sum(number_participants)) %>% 
  ggplot() +
  geom_col(aes(total_trained, event_title),
